  ...
  <div class="mb-5">
    <Swiper ref="mySwiper" :options="swiperOption">
      <swiper-slide class="item" v-for="item in identical" :key="item.id">
        <div class="card border h-100 pointer" @click.prevent="getDetail(item.id)">
          <div class="bg-cover card-img-top card-img-height-sm"
          :style="{ backgroundImage: `url(${ item.imageUrl[0] })` }">
          </div>
          <div class="card-body border-top bg-dark text-center px-2 py-3">
            <h5 class="mb-0">
              <router-link :to="`/product/${item.id}`">{{ item.title }}</router-link>
            </h5>
            <div class="mt-2 d-lg-flex justify-content-center align-items-center">
              <del class="text-secondary mr-lg-2">
